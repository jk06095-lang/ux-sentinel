...
The interactive demo set also includes `demo/interactive-navigation-stop.html` with `demo/scenarios/interactive-navigation-stop.yaml`, which proves scenario-driven interactive audit stops remaining planned actions after unexpected navigation when `allow_navigation: false`. `demo/interactive-motion.html` with `demo/scenarios/interactive-motion.yaml` is an intentionally failing motion-audit fixture that proves per-action animation traces and motion findings are attached to the report, action trace, state graph, and contact sheet.
