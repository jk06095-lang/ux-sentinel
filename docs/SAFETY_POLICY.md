...
Planner decisions are written as `plannerClickDecision` and `plannerClickDecisionReason`; runtime decisions are written as `runtimeClickDecision` and `runtimeClickDecisionReason`. On action records, `clickDecision` and `clickDecisionReason` are compatibility aliases for the runtime decision. Prefer runtimeClickDecision and `runtimeClickDecisionReason` in new consumers. On click candidates, `clickDecision` and `clickDecisionReason` are compatibility aliases for the planner decision. Prefer plannerClickDecision and `plannerClickDecisionReason` in new consumers. `clickCandidates` in the action trace and trace manifest keep both planner and runtime fields so reports can show cases where the planner allowed a click but runtime skipped it because of cursor target drift, overlay blockage, stale target, identity mismatch, or navigation safety.

Target ids are stable during replanning. Existing `data-ux-sentinel-target-id` values are not overwritten; new interactive targets receive `t0001`, `t0002`, ... and new scroll targets receive `s0001`, `s0002`, .... Before any user-like scroll, hover, focus, or click, ux-sentinel verifies the live element's identity without scrolling. Non-dangerous structural changes to tag, role, href, `data-ux-role`, `data-ux-action`, or `data-ux-clickable` are skipped as `identity_mismatch`. Benign label drift such as counters, punctuation, whitespace, or safe state suffixes is recorded as `benign_label_change` and may continue. A live label that becomes destructive, payment, logout, removal, account-deletion, or irreversible is skipped as `dangerous_label_change` even when structural identity also changed; the trace still records `structuralSignatureMatches: false`.

Skipped actions must keep before/after screenshots, a visual diff, a screen map, DOM diff, accessibility diff when available, and a clear skip reason. Pre-scroll identity skips intentionally do not write pointer traces because no pointer movement occurred. Hover, focus, and click-capable actions that reach pointer movement still write `actions/aNNN-pointer-trace.json` so a reviewer can inspect cursor movement, hover-triggered overlays, target movement, and final hit-test drift. The contact sheet shows planned click decision, runtime click decision, runtime reason, target identity status, original/latest candidate identity, visual diff path, and pointer trace path so a reviewer can see what the runner did and what it refused to do.
