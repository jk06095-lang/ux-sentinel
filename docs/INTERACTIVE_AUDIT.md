...
ux-sentinel run demo/scenarios/interactive-agentic-states.yaml --url http://localhost:3000/interactive-agentic-states --interactive --max-actions 6
ux-sentinel run demo/scenarios/interactive-navigation-stop.yaml --url http://localhost:3000/interactive-navigation-stop --interactive --max-actions 2
ux-sentinel run demo/scenarios/interactive-navigation-allow.yaml --url http://localhost:3000/interactive-navigation-allow --interactive --max-actions 2
ux-sentinel run demo/scenarios/interactive-hover-block.yaml --url http://localhost:3000/interactive-hover-block --interactive --max-actions 1
ux-sentinel run demo/scenarios/interactive-motion.yaml --url http://localhost:3000/interactive-motion --interactive --max-actions 1
```
...
The contact sheet is the fastest human review surface: each action shows the target, bbox, before/after/diff screenshots, pointer trace link, pointer movement metadata, optional animation trace link, compact animation metadata, and finding detectors linked to that action. It also includes a reviewer answer matrix; severity, detector, rule-family, and confidence filters; an action timeline; state graph summary; static state graph map; state transition path; a safety log that links before/after/diff/screen-map evidence artifacts for each action; accessibility cross-check; animation audit section; bbox overlays; and UX principle mapping for evidence-backed findings. `trace-manifest.json` is the machine-readable bundle index for the same evidence: it links root artifacts, action evidence files, state/finding counts, planner metadata, and capability policy. The matrix directly answers what the agent did, what it clicked or avoided, what changed, which evidence supports the result, and which UX rule or fix should guide review. Per-action and global finding rows include evidence status, concrete evidence, user impact, suggested fix, and regression check so the sheet separates evidence-backed findings from ambiguous heuristic review prompts without opening the Markdown report first.
...
Skipped actions are represented in `action-trace.json` and `contact-sheet.html` with a clear skip reason. A target can be skipped when it disappears, detaches, becomes invisible, moves offscreen, when pointer approach reveals an overlay, or when the final hit-test drifts away from the intended target. The action trace also records the resolved capability policy, planner budget, every baseline click candidate's allow/skip decision, target category, planned reason, risk level, pointer trace path, optional animation trace path, per-action evidence completeness summary, and each performed action's safe-click decision and reason. `state-graph.json` links state nodes and action edges with before/after screenshots, visual diff screenshots, DOM diff files, accessibility diff files, pointer trace files, animation trace files when enabled, cursor movement summaries, open dialog/menu/popover state evidence, finding detectors, and attached finding summaries with evidence, user impact, suggested fix, regression check, and UX rule metadata. The `interactive-agentic-states` demo keeps a local regression gate for benign state continuation and depth-1 replanning: the verifier expects agentic mode to click a primary CTA, tab, menu trigger, help trigger, accordion, and a newly discovered control, then prove each state change through DOM diff text and state-graph edges. The `interactive-navigation-stop` demo keeps a local regression gate for navigation safety: the verifier expects `allow_navigation: false`, one clicked action, a URL-changing state edge, and report/action-trace/contact-sheet notes that remaining planned actions stopped. The `interactive-navigation-allow` demo keeps the positive-path gate with a real `a[href]` link: the verifier expects `allow_navigation: true`, two clicked actions, a URL-changing state edge, no stop note, destination-page DOM diff evidence, and contact-sheet evidence for the replanned destination action. The `interactive-hover-block` demo keeps a local regression gate for pointer-path safety: the verifier expects hover content to appear during cursor approach, final hit-test mismatch, skipped safe click evidence, action-linked `hover_content_blocks_trigger` and `cursor_target_drift` findings, and contact-sheet pointer evidence. The `interactive-motion` demo keeps a local regression gate for opt-in motion audit: the verifier expects a failing primary CTA motion scenario to write `actions/a001-animation-trace.json`, attach motion detector ids to the action, link the trace from `state-graph.json`, and show animation evidence in `contact-sheet.html`.
