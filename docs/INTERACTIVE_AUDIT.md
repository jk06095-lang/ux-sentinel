...
The contact sheet is the fastest human review surface: each action shows the target, bbox, before/after/diff screenshots, pointer trace path, optional animation trace path, and finding detectors linked to that action. It also includes severity, detector, and rule-family filters; an action timeline; state graph summary; a safety log that lists before/after/diff/screen-map evidence paths for each action; accessibility cross-check; animation audit section; bbox overlays; and UX principle mapping for evidence-backed findings.
