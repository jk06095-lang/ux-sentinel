...
The expanded detector batches add target-size, target-spacing, visible affordance, visible/accessibility label mismatch, unnamed dialog checks, unannounced status checks, empty/loading/dead-end recovery checks, focus-ring, focus-obscuring, destructive-action cue, click-feedback, primary-action visual hierarchy, and label/action consistency checks. These rely on screen-map bbox/style evidence, `aria-live`, `aria-labelledby`, `visualWeight`, `data-ux-action`, focus evidence, pointer hit-tests, or state diffs.
