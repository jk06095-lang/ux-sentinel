...
- action edges with action id, action type, target id, target category, before/after state ids, before/after screenshots, visual diff path, DOM diff path, accessibility diff path, pointer trace path, and finding detectors
...
The expanded detector batches add target-size, target-spacing, visible affordance, visible/accessibility label mismatch, unnamed dialog checks, dialog/modal escape checks, popover-over-primary checks, unannounced status checks, empty/loading/dead-end recovery checks, focus-ring, focus-obscuring, destructive-action cue, click-feedback, primary-action visual hierarchy, and label/action consistency checks. These rely on screen-map bbox/style evidence, `aria-live`, `aria-labelledby`, `aria-modal`, `visualWeight`, `data-ux-action`, focus evidence, pointer hit-tests, or state diffs.
