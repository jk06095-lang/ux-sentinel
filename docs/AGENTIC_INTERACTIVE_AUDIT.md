...
Animation traces are written as `actions/aNNN-animation-trace.json` only when motion audit is enabled. They record CSS transition/animation evidence, Web Animations API entries, risky properties, optional reduced-motion comparison, target bbox movement, and browser long-task markers when available. Action records and contact sheets also include a compact `animationTraceSummary` so reviewers can see motion risk without opening the full trace first. See [MOTION_AUDIT.md](MOTION_AUDIT.md).
