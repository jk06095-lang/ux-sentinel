...
- action edges with action id, action type, target id, target category, before/after state ids, before/after screenshots, visual diff path, DOM diff path, accessibility diff path, pointer trace path plus cursor movement summary, and finding detectors
